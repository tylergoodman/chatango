...
var Room = (function (_super) {
    ...
    Room.prototype._find_user_by_id = function (connection_id) {
        for (var iter = this.users.values(), i = iter.next(); i.done === false; i = iter.next()) {
            var user = i.value;
            for (var iter_1 = user._connection_ids.values(), i_1 = iter_1.next(); i_1.done === false; i_1 = iter_1.next()) {
                var id = i_1.value;
                if (id === connection_id) {
                    return user;
                }
    ...
    Room.prototype.__command__participant = function (status, connection_id, session_id, user_registered, user_temporary, no_idea, joined_at) {
        ...
        if (status === '2') {
            var old_user = this._find_user_by_id(connection_id);
